...
var EventContainerWrapper = function (_React$Component) {
  ...
  function EventContainerWrapper() {
    ...
    _this.handleMove = function (point, boundaryBox) {
      ...
      var end = _dates2.default.add(currentSlot, _dates2.default.diff(event.start, event.end, 'minutes'), 'minutes');
  ...
  EventContainerWrapper.prototype.handleResize = function handleResize(point, boundaryBox) {
    var start = void 0,
        end = void 0;
    ...
    if (direction === 'UP') {
      ...
      start = _dates2.default.min(currentSlot, slotMetrics.closestSlotFromDate(end, -1));
    } else if (direction === 'DOWN') {
      start = accessors.start(event);
      end = _dates2.default.max(currentSlot, slotMetrics.closestSlotFromDate(start));
    }
  ...
  EventContainerWrapper.prototype.render = function render() {
    ...
    if (startsBeforeDay && startsAfterDay) label = localizer.messages.allDay;else label = localizer.format({ start: start, end: end }, format);
