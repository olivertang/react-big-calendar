...
var eventTimes = function eventTimes(event, accessors) {
  ...
  var isZeroDuration = _dates2.default.eq(start, end, 'minutes') && start.getMinutes() === 0;
...
var WeekWrapper = function (_React$Component) {
  ...
  function WeekWrapper() {
    ...
    _this.handleMove = function (_ref, node) {
      ...
      var start = _dates2.default.merge(metrics.getDateForSlot((0, _selection.getSlotAtX)(rowBox, x, false, metrics.slots)), accessors.start(event));

      var end = _dates2.default.add(start, _dates2.default.diff(accessors.start(event), accessors.end(event), 'minutes'), 'minutes');

      _this.update(event, start, end);
    };

    _this._selectable = function () {
      var node = (0, _reactDom.findDOMNode)(_this).closest('.rbc-month-row, .rbc-allday-cell');
  ...
  WeekWrapper.prototype.update = function update(event, start, end) {
    var segment = (0, _eventLevels.eventSegments)(_extends({}, event, { end: end, start: start, __isPreview: true }), this.props.slotMetrics.range, _common.dragAccessors);
  ...
  WeekWrapper.prototype.handleResize = function handleResize(point, node) {
    ...
    if (direction === 'RIGHT') {
      if (cursorInRow) {
        ...
        // add min
        end = _dates2.default.add(metrics.getDateForSlot((0, _selection.getSlotAtX)(rowBox, point.x, false, metrics.slots)), 1, 'day');
      } else if (_dates2.default.inRange(start, metrics.first, metrics.last) || rowBox.bottom < point.y && +metrics.first > +start) {
      ...
      } else if (_dates2.default.inRange(end, metrics.first, metrics.last) || rowBox.top > point.y && +metrics.last < +end) {
  ...
  WeekWrapper.prototype.render = function render() {
    ...
    return _react2.default.createElement(
      ...
      children,
      segment && _react2.default.createElement(_EventRow2.default, _extends({}, this.props, {
        selected: null,
